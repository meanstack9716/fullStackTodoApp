...
// const Todo = require('../models/todo'); //mongoDb todo model
// const PushSubscription = require('../models/pushSubscription'); //mongo db push model
const { TodoStatus } = require('../enums/todoStatus');
const { Op } = require("sequelize");
const Todo = require("../model/todoModel"); // Sequelize todo  model (for MySQL)
const PushSubscription = require('../model/pushSubscriptionModel'); // mysql push model
  ...
  cron.schedule('0 */2 * * *', async () => {
    try {
      ...
      const todos = await Todo.findAll({
        where: {
          expireAt: { [Op.between]: [now, sixHoursLater] },
          status: { [Op.ne]: TodoStatus.Completed },
        },
      });
      ...
      for (const todo of todos) {
        ...
        //  Sequelize (MySQL)
        const subs = await PushSubscription.findAll({ userId: todo.user });
